...
public class ElevensBoard extends Board {
	...
	private boolean playPairSum11IfPossible() {
		...
		if(findPairSum11(cardIndexes())){
			for (int sk1 = 0; sk1 < cardIndexes().size(); sk1++) {
				int k1 = cardIndexes().get(sk1).intValue();
				for (int sk2 = sk1 + 1; sk2 < cardIndexes().size(); sk2++) {
					int k2 = cardIndexes().get(sk2).intValue();
					if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
						return true;
					}
				}
